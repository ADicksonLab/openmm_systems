...
#+begin_src bash
pip install --index-url https://test.pypi.org/simple/ --no-deps OpenMM-Systems-salotz
#+end_src
...
After you tag the release you need to rebuild everything so that
~versioneer~ gets things right since it gets the version based on the
tags.
...
Then you can publish this build.
